...
public interface ClassAggregationMapper {
    ...
    List<ClassAggregationResultDto> selectExistingAggregations(
        @Param("classId") Long classId,
        @Param("gender") Integer gender,
        @Param("supportType") Integer supportType
    );

    void updateAggregationEndDate(
        @Param("classId") Long classId,
        @Param("gender") Integer gender,
        @Param("supportType") Integer supportType,
...
// Service Interface
public interface ClassAggregationService {
    void executeAggregation(Long yearId, LocalDate baseDate);
}

// Service Implementation
@Service
@RequiredArgsConstructor
public class ClassAggregationServiceImpl implements ClassAggregationService {
    ...
    public void executeAggregation(Long yearId, LocalDate baseDate) {
        List<ClassAggregationResultDto> newResults = mapper.selectClassAggregationResult(yearId, baseDate);

        for (ClassAggregationResultDto newDto : newResults) {
            List<ClassAggregationResultDto> existing = mapper.selectExistingAggregations(
                newDto.getClassId(), newDto.getGender(), newDto.getSupportType()
            );

            if (existing.isEmpty()) {
                newDto.setStartDate(baseDate);
                newDto.setEndDate(baseDate);
                mapper.insertSingleAggregation(newDto);
            } else {
                ClassAggregationResultDto last = existing.get(existing.size() - 1);
                if (last.getDataCount().equals(newDto.getDataCount())) {
                    mapper.updateAggregationEndDate(
                        newDto.getClassId(), newDto.getGender(), newDto.getSupportType(), baseDate);
                } else {
                    newDto.setStartDate(baseDate);
                    newDto.setEndDate(baseDate);
                    mapper.insertSingleAggregation(newDto);
...
<mapper namespace="com.example.mapper.ClassAggregationMapper">
    ...
    <select id="selectExistingAggregations" resultType="com.example.dto.ClassAggregationResultDto">
        SELECT
            学年組編成_学級クラスID AS classId,
            性別区分 AS gender,
            特別支援区分 AS supportType,
            データ件数 AS dataCount,
            開始年月日 AS startDate,
            終了年月日 AS endDate
        FROM GR_CLS_ORGNZ_CLS_CLS_POS_STDT
        WHERE 学年組編成_学級クラスID = #{classId}
          AND 性別区分 = #{gender}
          AND 特別支援区分 = #{supportType}
        ORDER BY 開始年月日
    </select>

    <update id="updateAggregationEndDate">
        UPDATE GR_CLS_ORGNZ_CLS_CLS_POS_STDT
        SET 終了年月日 = #{endDate},
